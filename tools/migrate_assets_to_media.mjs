...
const archiveInput = resolve(argument("--archive-input", join(repositoryRoot, "data", "catalog-archive.json")));
const archiveOutput = resolve(argument("--archive-output", join(mediaRoot, "catalog-archive.json")));
const manifestOutput = argument("--manifest-output", "");
const payload = JSON.parse(await readFile(input, "utf8"));

async function migrateCatalog(catalog) {
  ...
  catalog.generatedFrom = "Asset-to-Linux-media migration";
  return catalog;
}

await migrateCatalog(payload);
await mkdir(dirname(output), { recursive: true });
await writeFile(output, `${JSON.stringify(payload, null, 2)}\n`);
let archive = null;
try {
  archive = await migrateCatalog(JSON.parse(await readFile(archiveInput, "utf8")));
...
console.log(JSON.stringify({ products: payload.products.length, archived: archive?.products.length || 0, mediaRoot, output, archiveOutput: archive ? archiveOutput : null }));
